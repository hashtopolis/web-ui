...
describe('EditNotificationComponent', () => {
  ...
  const mockService: Partial<GlobalService> = {
    ...
    get(serviceConfig, _id: number, _routerParams?): Observable<any> {
      return of([]);
    },
    // Simulate the 'update' method to return an empty observable.
    // eslint-disable-next-line @typescript-eslint/no-unused-vars, @typescript-eslint/no-explicit-any
    update(serviceConfig, _id: number, _object: any): Observable<any> {
      return of({});
    }
  };
  ...
  it('should not allow action to be edited', () => {
    spyOn(mockService, 'get').withArgs(SERV.NOTIFICATIONS, 1).and.returnValue(of(nodificationResponse));
