...
export function LruCacheable(property: string[] | undefined = undefined): MethodDecorator {
  return function (target: object, propertyKey: string | symbol, descriptor: PropertyDescriptor) {
    ...
    descriptor.value = function (...args: any[]): object {
      ...
      const attributeValues = myProperties
        .map((key) => args[0][key])
        .join('_')
        .replaceAll(' ', '_');
      const cacheKey = `${propertyKey.toString()}_${attributeValues}`;
