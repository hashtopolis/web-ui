...
export class HealthChecksComponent implements OnInit {
  ...
  onDelete(id: number, name: string){
    this.alert.deleteConfirmation(name,'Health Checks').then((confirmed) => {
      if (confirmed) {
        // Deletion
        this.gs.delete(SERV.HEALTH_CHECKS, id).subscribe(() => {
          // Successful deletion
          this.alert.okAlert(`Deleted Health Check ${name}`, '');
          this.onRefreshTable(); // Refresh the table
        });
      } else {
        // Handle cancellation
        this.alert.okAlert(`Health Check ${name} is safe!`,'');
      }
    });
  }
